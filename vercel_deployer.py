...
class VercelDeployer:
    ...
    def __init__(self, api_token: str, team_slug: str, project_base: str):
        ...
        self._team_param = {"teamId": None}  # resolved on first call

    def _resolve_team(self):
        if self._team_param["teamId"]:
            return self._team_param["teamId"]
    ...
    def deploy_redirector(self, target_host: str, target_port: int, suffix: str) -> dict:
        ...
        #    - index.html: legit-looking landing page (cover)
        #    - vercel.json: rewrites ALL /api/* → Fly VM (reverse proxy)
        target_url = f"http://{target_host}:{target_port}"

        files = [
            {
                "file": "index.html",
                "data": self._cover_page_html()
            },
            {
                "file": "vercel.json",
                "data": self._vercel_config(target_url)
            },
            {
                "file": "package.json",
                "data": '{"name":"app","version":"1.0.0","private":true}'
            }
        ]
            ...
            "headers": [
                {
                    ...
                    "headers": [
                        ...
                        {"key": "Server", "value": "cloudflare"}  # spoof server header
                    ]
                }
            ]
        }, indent=2)
...
<p style="margin-top:40px;font-size:0.8rem;color:#999">&copy; 2024 CloudSync. All rights reserved.</p>
    ...
    def _wait_ready(self, deployment_id: str, team_id: str, timeout: int = 120):
