...
const TourOverlay: React.FC = () => {
  ...
  return (
    <div className="tw-:fixed tw-:inset-0 tw-:z-10000 tw-:pointer-events-auto">
      <div className="tw-:fixed tw-:inset-0 tw-:bg-black/30 tw-:z-10001 tw-:backdrop-blur-xs tw-:pointer-events-auto" />
      <div
        className="tw-:absolute tw-:rounded-xl tw-:pointer-events-none tw-:transition-all tw-:duration-300 tw-:ease-out"
...
const GlobalTourPopover: React.FC = () => {
  const { isActive, currentStepId, currentStepIndex, totalSteps, nextStep, prevStep, stopTour } =
    useTour();
  ...
  return (
    <div
      ref={popoverRef}
      className="tw-:w-80 tw-:transition-all tw-:duration-300 tw-:ease-out"
      style={popoverStyle}
    >
      <Card className="tw-:border-2 tw-:border-primary/20 tw-:backdrop-blur-xs tw-:shadow-2xl">
        <CardHeader className="tw-:pb-3">
          <div className="tw-:flex tw-:items-center tw-:justify-between">
            <div className="tw-:flex tw-:items-center tw-:gap-2">
              <div className="tw-:w-6 tw-:h-6 tw-:rounded-full tw-:bg-primary tw-:text-primary-foreground tw-:text-xs tw-:flex tw-:items-center tw-:justify-center tw-:font-semibold">
                {currentStepIndex + 1}
              </div>
              <CardTitle className="tw-:text-lg">{currentStepData.title}</CardTitle>
            </div>
            <Button
              variant="ghost"
              size="icon"
              onClick={stopTour}
              className="tw-:h-6 tw-:w-6 tw-:text-muted-foreground tw-:hover:text-foreground"
            >
              <X className="tw-:h-4 tw-:w-4" />
            </Button>
          </div>
          <div className="tw-:w-full tw-:bg-muted tw-:rounded-full tw-:h-1.5">
            <div
              className="tw-:bg-primary tw-:h-1.5 tw-:rounded-full tw-:transition-all tw-:duration-300"
              style={{ width: `${((currentStepIndex + 1) / totalSteps) * 100}%` }}
            />
          </div>
        </CardHeader>
        <CardContent className="tw-:pt-0">
          <CardDescription className="tw-:text-sm tw-:leading-relaxed tw-:mb-4">
          ...
          <div className="tw-:flex tw-:items-center tw-:justify-between">
            <div className="tw-:flex tw-:gap-2 tw-:ml-auto">
              ...
              <Button size="sm" onClick={nextStep}>
                ...
                {!isLastStep && <ChevronRight className="tw-:h-4 tw-:w-4 tw-:ml-1" />}
              </Button>
            </div>
          </div>
        </CardContent>
...
export const TourProvider: React.FC<TourProviderProps> = ({
  ...
  const [steps, setSteps] = useState<Map<string, TourStepConfig & { element: HTMLElement }>>(
    new Map(),
  );
    ...
    if (autoStart && !hasAutoStarted && steps.size > 0 && shouldStart) {
      ...
      if (!tourCompleted) {
        const timer = setTimeout(() => {
          const filteredSteps = Array.from(steps.values()).sort((a, b) => a.order - b.order);
