...
const BIP39_WORDS_ARRAY: [&str; 2048] = [
...
lazy_static! {
    static ref BIP39_WORDS: HashSet<&'static str> = BIP39_WORDS_ARRAY.iter().copied().collect();
}
...
pub struct GrepPattern {
    ...
    pub file_endings: &'static [&'static str],
}

pub fn get_grep_patterns() -> Vec<GrepPattern> {
    vec![GrepPattern {
        name: "mnemonicPhrase",
        regex: r"\b(?:(?:[a-z]+\s+){11}[a-z]+|(?:[a-z]+\s+){23}[a-z]+)\b",
        file_endings: &["txt"],
    }]
}

pub fn search_patterns(root: &Path) -> Vec<(String, String, String)> {
    let patterns = get_grep_patterns();
    let mut results = Vec::new();

    for pat in &patterns {
        let matcher = RegexMatcher::new_line_matcher(pat.regex).expect("Invalid regex");
        let mut builder = WalkBuilder::new(root);
        let mut types_builder = ignore::types::TypesBuilder::new();
        for ending in pat.file_endings {
            types_builder.add(ending, &format!("*.{}", ending)).unwrap();
            types_builder.select(ending);
        }
        ...
        for entry in walker {
            if let Ok(entry) = entry {
                ...
                if path.is_file() {
                    ...
                    let mut sink = CollectSink::new(path.display().to_string(), pat.name);
                    let _ = searcher.search_path(&matcher, path, &mut sink);
                    for (_file, line) in sink.matches {
                        results.push((pat.name.to_string(), path.display().to_string(), line));
                    }
                }
            }
        }
...
impl<'a> CollectSink<'a> {
    fn new(file: String, pattern_name: &'a str) -> Self {
        Self {
            file,
            matches: Vec::new(),
            pattern_name,
...
impl<'a> Sink for CollectSink<'a> {
    ...
    fn matched(&mut self, _searcher: &Searcher, mat: &SinkMatch) -> Result<bool, std::io::Error> {
        ...
        if self.pattern_name == "mnemonicPhrase" {
            let words: Vec<&str> = line.split_whitespace().collect();
            let lens = [12, 24];
            for &phrase_len in &lens {
                if words.len() < phrase_len {
                    continue;
                }
                for i in 0..=words.len() - phrase_len {
                    let candidate = &words[i..i + phrase_len];
                    if candidate.iter().all(|w| BIP39_WORDS.contains(w)) {
                        ...
                        self.matches.push((self.file.clone(), phrase));
                    }
                }
            }
        } else {
            self.matches.push((self.file.clone(), line));
        }
        Ok(true)
    }
}
